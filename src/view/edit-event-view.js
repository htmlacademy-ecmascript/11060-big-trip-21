...
import { humanizeDate, isChecked } from '../utils.js';
import { BLANK_EVENT } from '../mock/event.js';
      ...
      <div class="event__available-offers">
        ${offers.map((offer) => `
          <div class="event__offer-selector">
            <input class="event__offer-checkbox  visually-hidden" id="event-offer-luggage-1" type="checkbox" name="event-offer-luggage" ${isChecked(offer.checked)}>
...
function createDestinationTemplate(destination, photos) {
  return destination ? `
...
function createEditEventTemplate(event) {
  const {type, destination, offers, price, startTime, endTime} = event;
  const {description, name, pictures} = destination;

  const startEventTime = humanizeDate(startTime, DATE_FORMAT.EDIT_DATE);
  const endEventTime = humanizeDate(endTime, DATE_FORMAT.EDIT_DATE);
